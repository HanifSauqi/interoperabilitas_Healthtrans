      ...
      <table class="table-fixed w-full divide-y divide-gray-200 rounded-lg">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="ambulance in filteredAmbulances" :key="ambulance.id">
            ...
            <td class="px-6 py-4 whitespace-nowrap">{{ ambulance.petugas.no_telp }}</td>
            <td class="px-6 py-4 whitespace-nowrap">
              <span :class="['px-2 py-1 rounded text-center font-bold text-xs uppercase', getStatusClass(ambulance.status.toUpperCase())]">
                {{ ambulance.status }}
              </span>
            </td>
            <td class="px-6 py-4 whitespace-nowrap flex space-x-2">
              <router-link :to="'/admin/edit-ambulans/' + ambulance.id" class="text-blue-600 hover:text-blue-900">
              ...
              <button @click="confirmDeleteAmbulance(ambulance.id)" class="text-red-600 hover:text-red-900">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
...
export default {
  ...
  computed: {
    filteredAmbulances() {
      if (this.filter) {
        return this.ambulances.filter(ambulance => ambulance.lokasi.toLowerCase().includes(this.filter.toLowerCase()));
      }
  ...
  methods: {
    getStatusClass(status) {
      switch (status) {
        ...
        case "BEROPERASI": return "bg-purple-200 text-purple-800";
    ...
    async fetchAmbulances() {
      try {
        const response = await axios.get('http://127.0.0.1:8000/api/ambulans');
        this.ambulances = response.data.data;
        console.log(this.ambulances)
      } catch (error) {
        console.error("There was an error fetching the ambulances:", error);
      }
    },
    addAmbulance() {
    ...
    editAmbulance(ambulance) {
      ...
      async deleteAmbulance(id) {
        try {
          ...
          await axios.delete(`http://127.0.0.1:8000/api/ambulans/${id}`);
          ...
          console.log(`Ambulance with ID ${id} deleted successfully.`);
        } catch (error) {
          console.error(`Error deleting ambulance with ID ${id}:`, error);
          // Optionally show an error message or handle errors gracefully
        }
      },
      confirmDeleteAmbulance(id) {
        if (confirm("Are you sure you want to delete this ambulance?")) {
          this.deleteAmbulance(id);
        }
      },
    resetFilter() {
      this.filter = "";
